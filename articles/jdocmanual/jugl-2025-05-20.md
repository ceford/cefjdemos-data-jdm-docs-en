...
The source text for Jdocmanual is stored in files GitHub flavoured Markdown. Jdocmanual contains tools to edit and otherwise manage source content - more on that under Stashes later. However, I now prefer to use VSCode or VSCodium:

![help start here in markdown](../../../en/images/jdocmanual/jugl-vscodium-help-start-here.png)

- Explain Markdown: simple, clean text with a few markup symbols.
...
- There is a simple method to control nesting. Originally, only one nesting level was used. More nesting leads accessibility problems.
...
This screenshot shows my live site with an extra column pf buttons to **Pull** any changes from GitHub. So the normal procedure is to Pull then Build.
...
With translations there are a total of 5175 articles.